...
class EndpointMiner(Miner):
    ...
    def prompt(self, synapse: Prompting) -> Prompting:
        ...
        try:
            ...
            response = requests.post(self.config.endpoint.url, data=json.dumps({"user_input": str(messages), "verify_token":self.config.endpoint.verify_token}), headers= {"Content-Type": "application/json"}, timeout=12)
